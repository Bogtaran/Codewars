"""
Ситуация
Вас наняла компания, производящая электрические гаражные ворота. Несчастные случаи с нынешней линейкой продукции
привели к многочисленным повреждениям автомобилей, переломам и гибели нескольких домашних животных.
Ваша задача — написать более безопасную версию программного обеспечения для их контроллера.

Спецификация
Мы всегда начинаем с закрытой двери. На пульте дистанционного управления есть ровно одна кнопка, которая работает
 следующим образом.
...
Например, '..P....' означает, что в течение двух секунд ничего не происходит, затем нажимается кнопка, после чего
 никаких дальнейших событий не происходит.

Выходной сигнал
Строка, в которой каждый символ обозначает одну секунду и указывает положение двери (0 — полностью закрыта,
5 — полностью открыта). Дверь начинает двигаться немедленно, поэтому её положение меняется в ту же секунду, что
и событие.
